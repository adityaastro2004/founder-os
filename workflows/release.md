...
- A **[security](../agents/security.md) Pass** for any change touching auth, secrets,
...
|---|-------|-------|----------|
| 1 | Pre-flight | [eng-reviewer](../agents/reviewer.md) | confirm quality gates pass; no unrelated churn in the diff |
| 2 | Security gate | [eng-security](../agents/security.md) | Pass (a blocker here stops the release) |
| 3 | Verify build/tests | [eng-qa](../agents/qa.md) | `turbo build`, `turbo lint`, `turbo check-types`, backend `test_*.py` all green (output shown) |
| 4 | Migrate | [eng-executor](../agents/executor.md) | `alembic upgrade head` applied/validated where relevant |
| 5 | Publish | human-gated | branch + PR (or merge) — **only when the founder asks** |
| 6 | Completion report | [eng-product](../agents/product.md) | report in [reports/](../reports/); move task to `tasks/completed/`; roadmap → Shipped |
