...
| Date | Item | Task |
|------|------|------|
| 2026-08-19 | SEO round two (task 030, ADR-020): the site declared keywords it had no page for, so 10 URLs were added in three intent clusters — `/pricing` (prices mirrored from the `subscription_plans` seed, `Product` + `AggregateOffer` + per-plan `Offer`), `/integrations` + `obsidian`/`notion`/`google-calendar` (status and data direction taken from the adapters' real `capabilities` flags, `HowTo` setup schema), and `/compare` + `vs-chatgpt`/`vs-notion-ai`/`vs-virtual-assistant` (each required to state when the alternative wins). **`/` is now static** — the signed-in redirect moved from the page component to `proxy.ts`, since an `await auth()` there made the most-crawled URL dynamic. Plus `/llms.txt` (derived, `force-static`), env-gated Search Console / Bing verification, per-page `sitemap` dates, answer-engine crawlers named in `robots.txt`, and `Person`/`ItemList`/`HowTo` schema builders. Page content now lives in typed data modules that the pages, the sitemap, `llms.txt` and the JSON-LD all read — **but `lib/pricing.ts` mirrors the `subscription_plans` seed and `lib/integrations.ts` mirrors adapter `capabilities`; change the backend fact and the marketing module in the same commit** | [tasks/completed/030](../tasks/completed/030-seo-intent-pages.md) |
| 2026-08-17 | Public marketing site + SEO layer (task 029, ADR-019): the single-hero landing page became a 10-URL MPA in a new `(marketing)` route group — home (with a 634-word SEO write-up), `/features`, `/case-studies` + 3 illustrative scenario pages, `/about`, `/faq`, `/contact`, `/thank-you`, `/privacy`, `/terms`, plus a custom 404 that now actually returns 404 (unknown paths used to 307 anonymous visitors to `/sign-in`). `lib/site.ts` is the single metadata factory (unique title/description/canonical/OG per page) and owns the schema.org builders (Organization, WebSite, SoftwareApplication, local `ProfessionalService`, FAQPage, BreadcrumbList, Article, ContactPage); `next/og`-generated share card; `robots.txt` + `sitemap.xml` + webmanifest; sticky mobile CTA; GA4 env-gated alongside PostHog. Proxy page protection is now an explicit allowlist — **new authenticated pages must be added to `isProtectedPage`** | [tasks/completed/029](../tasks/completed/029-public-marketing-site-seo.md) |
| 2026-07-27 | Durable orchestrator (ADR-017): the orchestrator control loop is now a LangGraph `StateGraph` (`app/agents/graph/`) with a Postgres `AsyncPostgresSaver` checkpointer — crash-resume + human-in-the-loop (`POST /api/agents/orchestrate/resume`) that survive restarts. Routing = one cheap-tier classify + zero-token conditional edges (COMPLEX escape-hatches to LLM routing) for fewer orchestrator tokens; a `hydrate_context` node refreshes company/task state on resume. No LangChain models — specialists, tools, memory, event bus, and the 3-tier provider fallback are untouched (SSE contract parity-tested). In-place swap; checkpoint tables via migration `0003`. 27 graph/durability/mapping unit tests | [spec](superpowers/specs/2026-07-27-langgraph-orchestrator-design.md) · [plan](superpowers/plans/2026-07-27-langgraph-orchestrator.md) |
| 2026-07-21 | Global search command palette (task 024): replaced the dead decorative header input with a centered ⌘K trigger + real cross-entity search — new read-only user-scoped `GET /api/search` (ILIKE over tasks/knowledge/content-ideas/automations, wildcards escaped, title-ranked), keyboard-first palette (client-side page nav + recent queries + "ask the Orchestrator" fallback), deep-links to `?task=` / `?q=`; 9 helper unit tests + `test_search.py` integration suite | [tasks/completed/024](../tasks/completed/024-global-search-command-palette.md) |
| 2026-07-21 | Dark mode + UI correctness sweep (ADR-016): `.dark` token re-values with warm charcoal palette + owned ThemeProvider/toggle (sidebar + landing, pre-paint script, theme-aware Clerk), markdown rendering for agent chat (react-markdown kit component, XSS-safe), `[object Object]` API errors flattened to readable text everywhere, onboarding numeric bounds front+back (10 new unit tests), SQLi/XSS audit clean, overflow guards | [tasks/completed/022](../tasks/completed/022-dark-mode-and-ui-fixes.md) |
